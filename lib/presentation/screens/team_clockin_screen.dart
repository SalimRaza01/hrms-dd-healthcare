...
class _TeamClockinScreenState extends State<TeamClockinScreen> {
  ...
  void initState() {
    ...
    super.initState();
    // print(DateFormat('MMMM yyyy').format(DateTime.now()).toString());
    // attendenceLog = fetchAttendence(widget.empID,
    //     DateFormat('MMMM yyyy').format(DateTime.now()).toString());
    attendenceLog = fetchAttendence(widget.empID, 'May 2025');
  }
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                  ...
                  child: Text(
                    'Attendance Of ${DateFormat('MMMM yyyy').format(DateTime.now()).toString()}',
                      ...
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            scheduledTime =
                                _authBox.get('lateby').toString().contains('10')
                                    ? DateTime.parse(
                                    ...
                                    Stack(
                                      ...
                                      children: [
                                        if ((item.weekOff != 1 &&
                                            ...
                                            item.duration < 510))
                                          _buildTag(
                                              height, 'Half-Day', Colors.redAccent),
                                        if ((item.weekOff != 1 &&
                                            item.isLeaveTaken == false &&
                                            lateMinutes >= 1 &&
                                            lateMinutes <= 15))
                                        ...
                                        if (item.weekOff != 1 &&
                                          ...
                                          _buildTag(
                                              height, item.leaveType, Colors.green),
                                        if ((item.weekOff != 1 &&
                                            item.isLeaveTaken == false &&
                                            item.absent == 1 &&
                                            punchIn == "00:00"))
                                          _buildTag(
                                              height, 'Absent', Colors.redAccent),
                                      ],
                                    ),
                                  ],
                                ));
